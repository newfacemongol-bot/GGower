...
export default function BotMessagesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <MessageSquare className="w-6 h-6" /> Бот мессежүүд
          </h1>
      ...
      {loading ? (
        <div className="text-center text-slate-500 py-16">Ачаалж байна...</div>
      ) : (
        <div className="space-y-4">
          {items.map((item) => (
            <div key={item.key} className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              />
              {dirty[item.key] && (
                <div className="text-xs text-amber-600 mt-1">Хадгалаагүй өөрчлөлт</div>
              )}
            </div>
          ))}
        </div>
